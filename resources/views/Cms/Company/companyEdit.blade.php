    ...
    <div class="col-md-12 ">
      <div  class="panel panel-default">
        ...
        <div class="panel-body">
          <hr>
          <form method="post" action="{{url('cms/company/update')}}" role="form" id="form1">
            {{ csrf_field() }}
            <div class="row">
              <div class="col-md-12">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input value="{{ $results->nm_bd }}" type="text" placeholder="{{trans('common.nameCompany')}}" class="form-control" id="companyName" name="companyName">
                 <input value="{{ $results->id_bd }}" type="hidden" id="id" name="id">
                </div>
                <div class="form-group">
                  <label class="control-label"> {{trans('common.companyNumber')}}<span class="symbol required">*</span> </label>
                  <input value="{{ $results->relnr_bd }}" type="text" placeholder="{{trans('common.companyNumber')}}" class="form-control" id="companyNumber" name="companyNumber">
                </div>
                <div class="form-group">
                ...
                @if (count($kindOfCompany) > 0)
                 @foreach ($kindOfCompany as $val)
                      ...
                      @if(strtolower($val['id']) == strtolower($results->ondvorm_bd))
                ...
                <div class="form-group">
                 ...
                 @foreach ($relationship as $val)
                   ...
                   @if(strtolower($val->id_rm) == strtolower($results->id_rm))
                  <option selected value="{{$val->id_rm}}">{{$val->vl_rm}} {{$val->nm_rm}}</option>
                  @else
                  <option value="{{$val->id_rm}}">{{$val->vl_rm}} {{$val->nm_rm}}</option>
                  @endif
