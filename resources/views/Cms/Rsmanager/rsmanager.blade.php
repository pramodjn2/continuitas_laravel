    ...
    <div id="content">
      <div id="sortable-panel" class="">
        <div id="titr-content" class="col-md-12">
          ...
          <h5>&nbsp;</h5>
          <div class="actions">
           <a href="{{url('cms/manager/add')}}" class="btn btn-success ">{{trans('common.addnew')}} </a>
           <a href="{{url('cms/manager/export')}}" class="btn btn-primary ">{{trans('common.export')}}</a>
           <a href="{{url('cms/manager/import')}}" class="btn btn-primary ">{{trans('common.import')}}</a> 
          </div>
        </div>
        <div class="col-md-12 ">
          <div  class="panel panel-default">
            ...
            <div class="panel-body">
              <table id="example1" class="table table-striped table-bordered width-100 cellspace-0" >
                ...
                <tr>
                  <td>{{ ucfirst($val->vl_rm) }}</td>
                  <td>{{ ucfirst($val->nm_rm) }}</td>
                  <td>{{ ucfirst($val->sexe_rm) }}</td>
                  <td>{{ ucfirst($val->email_rm)	 }}</td>
                  <td>{{ ucfirst($val->status_rm) }}</td>
                 
                  <td class="center"><div class="visible-md visible-lg visible-sm visible-xs"> 
                  <a href="manager/edit/{{$val->id}}" class="btn btn-xs btn-teal tooltips" data-placement="top" data-original-title="Edit"> <i class="fa fa-edit"></i></a> 
                  
                 <a href="manager/view/{{$val->id}}" class="btn btn-xs btn-green tooltips" data-placement="top" data-original-title="Share"> <i class="fa fa-share"></i></a>
                  
                  <a onclick="return confirm('{{trans('common.confirmDelete')}}')" href="manager/delete/{{$val->id}}" class="btn btn-xs btn-bricky tooltips" data-placement="top" data-original-title="Remove"> <i class="fa fa-times fa fa-white"></i></a> </div></td>
                </tr>
                @endforeach
                  </tbody>
